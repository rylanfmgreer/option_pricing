...
{
    // precalculate some nice stuff that will be used later
    m_max_price = 2. * m_K;
    ...
    // Should this be in the initialization?
...
{
    // FIXME: is m_ds always an appropriate denominator?
...
{
    // create the boundary condition t=T in the array. 
...
void BlackScholesExplicit_I_Option::evolve_calc_array_backwards_by_one_timestep_explicit(int p_timestep)
{
    ...
    // For anything that isn't the end, calculate the price
    for( int i(1); i < m_calc_array.size()-1; ++i )
        new_calc_array[i] = calc_opt_price_from_theta(i);
    
    // Set the boundary conditions in stock price pace
    new_calc_array[0] = 0.;
    new_calc_array[new_calc_array.size() - 1] = (
        m_max_price - m_K) *  calculate_discount_factor((p_timestep + 1) * m_dt);

    // Save down our new array of option prices
